...
static uint8_t FIXTURE_USER_D_KEY[] = {
...
int main(int argc, char** argv)
{
  try {
    ...
    Name groupName("/Prefix/READ");
    Name userName("/U");
    Name userKeyName("/U/Key");
    Name contentPrefix("/Prefix/SAMPLE");
    Name contentName = Name(contentPrefix).append("Content");

    ptr_lib::shared_ptr<KeyChain> keyChain = createVerifyKeyChain();
    ...
    NacConsumerHandler handler
      (nameSpace, keyChain.get(), groupName, userName,
       ptr_lib::make_shared<Sqlite3ConsumerDb>(databaseFilePath));
    Blob fixtureUserDKeyBlob(FIXTURE_USER_D_KEY, sizeof(FIXTURE_USER_D_KEY));
